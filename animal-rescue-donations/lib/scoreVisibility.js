...
function calculateVisibilityScore(dog) {
  ...
  try {
    ...
    if (dog.breeds && dog.breeds.mixed) score += 8;

    // Special needs penalty (harder to place)
    if (dog.attributes && dog.attributes.special_needs) score += 15;
    ...
    const city = (dog.contact && dog.contact.address && dog.contact.address.city) ? 
                 dog.contact.address.city.toLowerCase() : '';
    ...
    const primaryBreed = (dog.breeds && dog.breeds.primary) ? 
                        dog.breeds.primary.toLowerCase() : '';
    const popularBreeds = [
    ...
    if (dog.description && (
        dog.description.toLowerCase().includes('medical') ||
    ...
    if (attributes.energy_level === 'High') score += 8;
